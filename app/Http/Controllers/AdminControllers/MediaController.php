...
class MediaController extends Controller
{
    ...
    public function storeThumbnial($Path, $filename, $directory, $input)
    {
        ...
        $namethumbnail = Image::make($originalImage)->resize($thumbnail_values[1]->value, $thumbnail_values[0]->value, function ($constraint) {
    ...
    public function storeMedium($Path, $filename, $directory, $input)
    {
        ...
        $namemedium = Image::make($originalImage)->resize($Medium_values[1]->value, $Medium_values[0]->value, function ($constraint) {
            $constraint->aspectRatio();
        })->save($destinationPath. 'medium' . time().$filename);
    ...
    public function storeLarge($Path, $filename, $directory, $input)
    {
        ...
        $namelarge = Image::make($originalImage)->resize($Large_values[1]->value, $Large_values[0]->value, function ($constraint) {
            $constraint->aspectRatio();
        })->save($destinationPath. 'large' . time().$filename);
