...
// ==============================================================================
//onchange get districts agains state
$(document).on('change', '#entry_state_id', function(e){
	// $("#loader").show();
	var state_id = $(this).val();
	$.ajax({
	  url: "{{ URL::to('admin/getdistricts')}}",
	  dataType: 'json',
	  type: "post",
	  // data: '&zone_country_id='+zone_country_id,
        data: {
	  ...
	  success: function(data){
		...
		if(data.data.length>0){
			var i;
			var showData = [];
			showData[0] = "<option value=''>Select District</option>";
			for (i = 0; i < data.data.length; ++i) {
				showData[i+1] = "<option value='"+data.data[i].districts_id+"'>"+data.data[i].districts_name+"</option>";
			}
            // $('.selectstate').show();
		...
		$(".districts_content").html(showData);
	  }
	});

});
//onchange get vidhan sabhas agains district
$(document).on('change', '#entry_district_id', function(e){
	// $("#loader").show();
	var district_id = $(this).val();
	$.ajax({
	  url: "{{ URL::to('admin/getvidhansabhas')}}",
	  dataType: 'json',
	  type: "post",
	  // data: '&zone_country_id='+zone_country_id,
        data: {
	  ...
	  success: function(data){
		...
		if(data.data.length>0){
			var i;
			var showData = [];
			showData[0] = "<option value=''>Select Vidhan Sabhas</option>";
			for (i = 0; i < data.data.length; ++i) {
				showData[i+1] = "<option value='"+data.data[i].vidhan_sabhas_id+"'>"+data.data[i].vidhan_sabhas_name+"</option>";
			}
            // $('.selectstate').show();
		...
		$(".vidhan_sabhas_content").html(showData);
	  }
	});
...
var element = $(this);
var user_id = element.attr('mdf-users-id');
var status = element.text() === "Active" ? 0 : 1;
	Swal.fire({
		title: 'Do you want to update the user status?',
		...
		confirmButtonText: 'Update',
			...
			if (result.isConfirmed) {
				$.ajax({
				type: 'POST',
				url: '{{ url('admin/mdf_users/change_status_mdf_user') }}',
				data: {
					"_token": "{{ csrf_token() }}",
					user_id: user_id,
					status: status
				...
				success: function(data) {
					console.log(data);
					element.removeClass(element.text() === "Active" ? 'badge bg-light-blue' : 'badge bg-red');
					element.addClass(element.text() === "Active" ? 'badge bg-red' : 'badge bg-light-blue');
					element.text(element.text() === "Active" ? 'Inactive' : 'Active')
